...
Kappale kertoo CASP-menetelmistä (Computer Aided Synthesis Planning) ja kuinka nämä liittyvät retrosyntetisointianalyysiin.
...
Tässä kappaleessa kerrotaan lyhyesti, minkälaista dataa käytetään koneppimismallien kouluttamisessa.
...
\begin{figure}[!ht]
    \centering
    \includegraphics[width=6.5cm, height=6.5cm]{melatonin-smiles.png}
...
Erilaisten yhdisteiden suuri lukumäärä esittää tarpeen tehokkaille algoritmeille ja menetelmille, jotka auttavat karsimaan tästä suuresta määrästä kemiallisia yhdisteitä vain lupaavimmat.
...
ZINC on ilmainen tietokanta, joka sisältää tietoa lääkeyhdisteistä, ja jota voidaan käyttää virtuaalisessa seulonnassa~\cite{ZINC}.
...
PPN-verkon rakenne ja toimintaperiaate näkyvät kuvassa~\ref{fig:cmg-model-ppn}.
...
ICHO-malli käyttää sekä samananlaista reaktiodataa kuin 3N-MCTS-malli, mutta se myös hyödyntää ammattikemistien luomia heuristisia sääntöjä.

\subsection{3N-MCTS-malli}

3N-MCTS on koneoppimismalli, jonka on kehittänyt Marwin et al.\ tutkimusryhmä.
Se etsii retrosyntetisointipolkuja yksinkertaisempiin ja saatavilla oleviin lähtöaineisiin~\cite{SeglerMarwinHS2018Pcsw}.
Kun retrosyntetisointipolku on varmennettu ja todettu toimivaksi, niin syötteenä annettu yhdiste on mahdollista syntetisoida laboratoriossa.
...
Näistä säännöistä saadaan selville, mitkä lähtöaineet voivat muodostaa annetun yhdisteen.
...
\begin{enumerate}
    \item Ensimmäisessä vaiheessa algoritmi valitsee seuraavan lupaavimman tilan puusta kunnes puun lehti on saavutettu.
          Jos lehdessä käydään ensimmäisen kerran valintavaiheen aikana, niin lehti arvostellaan simuloimalla hakualgoritmia \(d\) askelta eteenpäin samalla muodostaen syntetisointipolkua (Rollout).
    ...
    \item Kolmannessa vaiheessa tarkistetaan lehden tila.
          Jos lehti on `todistetusti toimiva', niin algoritmi palauttaa luvun, mikä on suurempi kuin yksi, jolloin lehteä suositellaan käytettävän syntetisointipolussa.
          Muussa tapauksessa lehdelle suoritetaan rollout-toiminto, jolloin RPN antaa rekursiivisesti uusia sääntöjä niin kauan, kunnes lehti on pilkottu lähtöaineisiin tai kunnes suurin sallittu syvyys \(d\) on saavutettu.

    \item Viimeisessä vaiheessa lehtien arvot päivitetään.
          Jos lähtöaineet löydetään rollout-toiminnon aikana, niin lehti saa palkinnoksi arvon 1.
...
Saattaa kuitenkin olla, että syntetisointipolkua ei voida luoda.
Joko syntetisointipolun tutkimiseen menee liian kauan aikaa tai syntetisointipolku sisältää liian monta vaihetta yhdisteen syntetisoimiseen.
...
On myös yleistä, että retrosyntetisointipolkuja suunnitelevat mallit hyödyntävät heurustiikkaa~\cite{ExpertKnowledgeRetorsynthesis}.
...
\begin{enumerate}
    \item Se laskee reaktioiden ilmaantumistodennäköisyyttä uudelleen perustuen kuinka usein ne ilmaantuvat ammattilaisten käyttämissä reaktioissa.
...
ICHO-mallin rakenne on esitetty kuvassa~\ref{fig:icho-neuralnetwork}.
Samalla tavalla kuin 3N-MCTS-mallissa, ICHO-mallissa käytetään myös ELU-aktivointifunktiota.
...
Tämä on haluttu lähestymistapa retrosyntetisointiin, koska se minimoi syntetisointivaiheiden määrän.
...
Kuvaajasta~\ref{fig:icho-performance} nähdään, että ICHO+ -malli pisteyttää kirjallisuuteen perustuvat muunnossäännöt paremmin kuin muut mallit, vaikkakin eroavaisuudet ovat vähäiset.
...
Tätä aihetta avattiin tutkielmassa, koska syntetisointisuunnittelu on osa lääkkeiden löytämis- ja kehitysvaihetta.
Tämän aiheen tutkiminen voi luoda uusia ja tehokkaampia tapoja, kuinka syntetisointia voidaan toteutaa käytännössä.
...
3N-MCTS on kokonaisempi malli, joka palauttaa yhdisteen syntetisointipolun.
...
ICHO-malli puolestaan palauttaa parhaimman muunnossäännön, jonka avulla yhdiste voidaan luoda.
Se ei etsi syntetisointireittejä, mutta se käyttää heuristiikkaa apunaan muunnossääntöjen etsimisessä.
Näiden mallien avaamisen tarkoituksena on näyttää, että syntetisointisuunnittelua voidaan suorittaa usealla tavalla ja kuinka eri tapojen suorituskykyä voidaan verrata toisiinsa.
...
Tutkimuksen ja kehitettyjen koneoppimismallien määrä näyttääkin viittaavan, että tulevaisuudessa koneoppiminen tulee olemaan merkittävä osa lääkekehitystä.
